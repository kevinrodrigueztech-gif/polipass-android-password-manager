...
public class Detalle_registro extends AppCompatActivity {
    ...
    private void MostrarInformacionRegistro() throws Exception {
        Password registro = helper.ObtenerRegistroPorId(id_registro);
        if (registro == null) {
            return;
        }

        String t_registro = registro.getT_registro();
        String t_actualizacion = registro.getT_actualiacion();

        Calendar calendar_t_r = Calendar.getInstance(Locale.getDefault());
        calendar_t_r.setTimeInMillis(Long.parseLong(t_registro));
        String tiempo_registro = DateFormat.format("dd/MM/yyyy hh:mm:aa", calendar_t_r).toString();

        Calendar calendar_t_a = Calendar.getInstance(Locale.getDefault());
        calendar_t_a.setTimeInMillis(Long.parseLong(t_actualizacion));
        String tiempo_actualizacion = DateFormat.format("dd/MM/yyyy hh:mm:aa", calendar_t_a).toString();

        D_Titulo.setText(registro.getTitulo());
        D_Cuenta.setText(registro.getCuenta());
        D_NombreUsuario.setText(registro.getNombre_usuario());
        D_Password.setText(registro.getPassword());
        ...
        D_SitioWeb.setText(registro.getSitio_web());
        D_Nota.setText(registro.getNota());
